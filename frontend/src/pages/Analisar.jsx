...
export default function Analisar({ onBack }) {
    ...
    return (
        <div style={{ color: "#fff", paddingBottom: "40px" }}>
            <style>{`
                .om-filter-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 20px; }
                .om-filter-item { display: flex; flexDirection: column; gap: 4px; }
                .om-select { background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.2); color: #fff; padding: 10px; border-radius: 12px; font-size: 12px; outline: none; appearance: none; }
                .om-suspect-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
                .om-suspect-card { position: relative; border-radius: 16px; overflow: hidden; border: 1px solid rgba(255,255,255,0.15); cursor: pointer; transition: transform 0.2s; background: #000; height: 180px; }
                .om-suspect-card:active { transform: scale(0.96); }
                .om-suspect-img { width: 100%; height: 100%; object-fit: cover; opacity: 0.8; }
                .om-suspect-name { position: absolute; bottom: 0; left: 0; right: 0; padding: 10px; background: linear-gradient(transparent, rgba(0,0,0,0.9)); font-size: 11px; font-weight: 800; text-align: center; color: #80bdff; text-transform: uppercase; }
                
                .om-modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.85); backdrop-filter: blur(5px); z-index: 2000; display: flex; align-items: center; justifyContent: center; padding: 20px; }
                .om-modal-content { max-width: 450px; width: 100%; max-height: 85vh; overflow-y: auto; background: #071a26; border: 1px solid rgba(128,189,255,0.3); border-radius: 24px; position: relative; padding: 0; }
                .om-detail-row { padding: 12px 15px; border-bottom: 1px solid rgba(255,255,255,0.05); }
                .om-detail-label { font-size: 10px; color: #80bdff; font-weight: 800; text-transform: uppercase; margin-bottom: 4px; letter-spacing: 1px; }
                .om-detail-value { font-size: 14px; opacity: 0.95; line-height: 1.4; }
